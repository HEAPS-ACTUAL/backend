...
async function createNewTest(email, testName, testType) {
  try {
    ...
    const sqlQuery =
      "Insert into Test (Email, TestID, TestName, TestType) values (?, ?, ?, ?)";
    const insertOk = await execute(sqlQuery, [
      email,
      testID,
      testName,
      testType,
    ...
    if (insertOk) {
      console.log(
        `${testName} (${testType}) with TestID: ${testID} added for ${email}!`
      );
      return testID;
    }
  } catch (error) {
...
async function determineTheNextTestID() {
  try {
    ...
    const returnedData = await execute(sqlQuery);
...
async function deleteTest(req, res) {
  ...
  try {
    ...
    console.log(
      `Could not delete ${testName} due to the following error: ${error}`
    );
...
async function getTestInfo(req, res) {
  ...
  try {
    ...
    const returnedData = await execute(sqlQuery, [email, testType, testStatus]);
...
function getPrompt(testType, difficulty, numOfQuestions = 12) {
  ...
  const promptDict = {
    ...
    Q: (
      numOfQuestions,
      difficulty
    ) => `Based on the text above, generate ${numOfQuestions} questions. These questions should test how well I know the content of the given text. The difficulty level of the questions should be ${difficulty}. \n\n
...
async function formatAndStoreTest(
  email,
  testName,
  testType,
  difficulty,
  ...
  try {
    ...
    if (
      array_of_question_obj_strings[
        array_of_question_obj_strings.length - 1
      ] === ""
    ) {
      array_of_question_obj_strings.pop();
    }
...
async function generateAndStoreTest(req, res) {
  try {
    ...
    const selectedPages = JSON.parse(req.body.selectedPages); // array of strings

    // console.log(email, testName, difficulty, testType, selectedPages);

    console.log("Extracting text now...");
    const extractedText = await extractTextFromPDF(uploadedFile, selectedPages); // FUNCTION IMPORTED FROM FILE CONTROLLER

    console.log(
      `Querying chatgpt for ${testType === "F" ? "flashcard" : "quiz"} now...`
    );
    const chatgptResponse = await queryChatgptForTest(
      extractedText,
      testType,
      difficulty
    ...
    const hasBeenStored = await formatAndStoreTest(
      email,
      testName,
      testType,
      difficulty,
